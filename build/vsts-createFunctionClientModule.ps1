...
$parentModule = 'Osprey'
if (-not $ModuleName) { $ModuleName = 'Osprey.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\Osprey'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\Osprey.psd1"
$prereqHash = @{
...
if ($LocalRepo)
{
	...
	Write-PSFMessage -Level Important -Message "Creating Nuget Package for module: Osprey"
	New-PSMDModuleNugetPackage -ModulePath $workingRoot.FullName -PackagePath . -EnableException
}
else
{
	# Publish to Gallery
	Write-PSFMessage -Level Important -Message "Publishing the Osprey module to $($Repository)"
