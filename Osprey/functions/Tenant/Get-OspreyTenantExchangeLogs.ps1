...
Function Get-OspreyTenantExchangeLogs {
    ...
    # If not null then we must have found some events so flag them
        ...
        Foreach ($rule in $TenantNewInboxRules) {
            ...
            $report | Out-MultipleFileType -fileprefix "InboxRules_New" -csv -append 

            $Investigate = $false #make sure flag is salse
            ...
            if ($Investigate -eq $true) {
                ...
                $report | Out-MultipleFileType -fileprefix "_Investigate_InboxRules_New" -csv -append
            }
        }
