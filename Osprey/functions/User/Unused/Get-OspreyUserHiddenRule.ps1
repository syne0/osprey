Function Get-OspreyUserHiddenRule { #TODO: determine if this is still nessecary, combine this into normal inboxrule function
    ...
    Get-OspreyUserHiddenRule -UserPrincipalName user@contoso.com -EWSCredential (get-credential)

    Searches user@contoso.com looking for hidden inbox rules using the provided credentials
    .EXAMPLE

    Get-OspreyUserHiddenRule -UserPrincipalName (get-mailbox -Filter {Customattribute1 -eq "C-level"})
